...
void CTSPAppMain::initDataResults()
{
    ...
    strNCpyUtil(tsp_title[n++], (char*)"testTSPAlgoCompletoOpenMP()", BIGSTRSZ);
    strNCpyUtil(tsp_title[n++], (char*)"testTSPAlgoCompletoOpenMPI()", BIGSTRSZ);
...
void CTSPAppMain::testTSPAlgoCompletoOpenMP()
{
    datatest_t* p_results = & m_results[DEF_TSP_ALGO_COMPLETO_OPENMP];
    ...
    CTSPAlgoCompletoOpenMP tspAlgoCompletoOpenMP(
        m_appid, 
        m_nprocs, 
        m_ftype, 
        p_results, 
...
void CTSPAppMain::testTSPAlgoCompletoOpenMPI()
{
    datatest_t* p_results = & m_results[DEF_TSP_ALGO_COMPLETO_OPENMPI];
    ...
    CTSPAlgoCompletoOpenMPI tspAlgoCompletoOpenMPI(
        m_appid, 
        m_nprocs, 
        m_ftype, 
        p_results, 
    ...
    tspAlgoCompletoOpenMPI.execute();
...
void CTSPAppMain::testTSPForcaBrutaOpenMP()
{
    ...
    if(m_appid == 0) {
        for(int i = 0; i < m_nprocs; i++) {
            datatokenptr_t* p_datares = m_datares[i];
            showResultData(p_datares, m_datasz);
        }
    }

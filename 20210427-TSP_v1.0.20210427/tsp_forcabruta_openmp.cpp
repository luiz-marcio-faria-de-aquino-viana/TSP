...
void CTSPForcaBrutaOpenMP::avaliateTotalCost(double* initial_cost, double* min_cost, datatokenptr_t* datares, datatokenptr_t* datawrk, long datasz)
{
...
long CTSPForcaBrutaOpenMP::executeStep(datatokenptr_t* datawrk, long datasz)
{
    long isRunning = TRUE;
...
#pragma omp parallel num_threads(DEF_NUM_OF_THREADS) private(nThreads, tid, tidLenVal, tidStartVal) shared(m_datares, m_datawrk, m_datasz, initial_cost, final_cost)
    {
        nThreads = omp_get_num_threads();
        tid = omp_get_thread_num();

        tidLenVal = (m_datasz / nThreads);
        tidStartVal = tid * tidLenVal;

        if(tid == (nThreads - 1))
            tidLenVal = m_datasz - tidStartVal;

        initial_cost[tid] = 0.0;
        final_cost[tid] = 0.0;

        datatokenptr_t* p_datares = m_datares[tid];
        datatokenptr_t* p_datawrk = m_datawrk[tid];

        resetResultData(p_datares, p_datawrk, m_datasz);
        ...
        {
            long bIsValid = validData(p_datawrk, m_datasz);
            if(bIsValid == TRUE) {
                avaliateTotalCost(& inicost, & fincost, p_datares, p_datawrk, m_datasz);
            }
            isRunning = executeStep(p_datawrk, m_datasz);
        }
    ...
    m_result->datatest_val.datavaltest_initial_cost = initial_cost[0];
    ...
    {
        if(final_cost[i] < m_result->datatest_val.datavaltest_final_cost) 
        {
            m_result->datatest_val.datavaltest_final_cost = final_cost[i];
...
void CTSPForcaBrutaOpenMP::debugWorkData(datatokenptr_t* datawrk, long datasz)
{
    double ctotal = 0.0;
